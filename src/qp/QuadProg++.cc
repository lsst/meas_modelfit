/* 
   ...
   Modified for inclusion in the LSST software framework (GPL v3), 2011 by Jim Bosch.
...
template<typename T>
void print_vector(const char* name, const Eigen::Matrix<T,Eigen::Dynamic,1,Eigen::AutoAlign>& v, int n = -1);
                      ...
                      Eigen::VectorXd& x, int maxIterations)
{
    ...
    for (i = 0; i < p; i++) {
        ...
        /* compute full step length t2: i.e., the minimum step in primal space s.t. the contraint 
           becomes feasible */
        t2 = 0.0;
        if (fabs(z.squaredNorm()) > std::numeric_limits<double>::epsilon()) // i.e. z != 0
...
inline void update_z(Eigen::VectorXd& z, const Eigen::MatrixXd& J, const Eigen::VectorXd& d, int iq)
{
    /* setting of z = H * d */
    if (iq < d.size())
        z = J.block(0, iq, J.rows(), J.cols() - iq) * d.segment(iq, d.size() - iq);
}
...
template<typename T>
void print_vector(const char* name, const Eigen::Matrix<T,Eigen::Dynamic,1,Eigen::AutoAlign>& v, int n)
{
    std::ostringstream s;
    std::string t;
    if (n == -1)
