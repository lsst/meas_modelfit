...
private:
    friend class ShapeletConvolution;
...
TripleProductIntegral::TripleProductIntegral(int order1, int order2, int order3) :
    ...
    _array(
        ndarray::allocate(
            ndarray::makeVector(
                afwShapelets::computeOffset(order1+1), 
                afwShapelets::computeOffset(order2+1),
                afwShapelets::computeOffset(order3+1)
            )
        )
    )
{
...
mfShapelets::ShapeletConvolution::Impl::Impl(
    ...
    _monomialFwd(
        Eigen::MatrixXd::Zero(
            afwShapelets::computeSize(_rowOrder),
            afwShapelets::computeSize(_rowOrder)
        )
    ),
...
void mfShapelets::ShapeletConvolution::Impl::evaluate(
    ndarray::Array<double,2,2> const & matrix,
    afw::geom::ellipses::BaseCore & ellipse
) const {
    ndarray::EigenView<double,2,2> result(matrix);
...
Eigen::MatrixXd mfShapelets::ShapeletConvolution::Impl::computeHermiteTransformMatrix(
    int order, lsst::afw::geom::LinearTransform const & transform
) const {
    int const size = afwShapelets::computeSize(order);
...
int mfShapelets::ShapeletConvolution::getRowOrder() const { return _impl->getRowOrder(); }

int mfShapelets::ShapeletConvolution::getColOrder() const { return _impl->getColOrder(); }

void mfShapelets::ShapeletConvolution::evaluate(
    lsst::ndarray::Array<double,2,2> const & matrix,
    lsst::afw::geom::ellipses::BaseCore & ellipse
) const {
    return _impl->evaluate(matrix, ellipse);
}
