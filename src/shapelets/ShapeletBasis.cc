...
void mfShapelets::ShapeletBasis::_evaluate(
    ...
    afwShapelets::EllipticalShapeletBasis basis(_order, afwShapelets::HERMITE, ellipse.getCore());
    ...
    for (; spanIter != spanEnd; ++spanIter) {
        ...
        for (int x = span.getX0(); x <= span.getX1(); ++x) {
            basis.fillEvaluationVector(
                *pixelIter,
                x - fullEllipse.getCenter().getX(), 
                span.getY() - fullEllipse.getCenter().getX()
            );
        }
    }
}
