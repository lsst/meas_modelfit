#ifndef NDARRAY_Expression_hpp_INCLUDED
#define NDARRAY_Expression_hpp_INCLUDED

/** 
 *  @file ndarray/Expression.hpp
 *
 *  \brief Definitions for Expression.
 */

#include "ndarray/detail/ExpressionTraits.hpp"
 ...
 *  \ingroup MainGroup
 *
 *  Expression is a CRTP base class for both true array objects (subclasses of
 ...
 *
 *  Expression also provides implementations for a few STL compatibility and
...
public:
    ...
    typedef typename detail::ExpressionTraits<Derived>::Element Element;
    /// \brief Number of dimensions (boost::mpl::int_).
    typedef typename detail::ExpressionTraits<Derived>::ND ND;
    /// \brief Nested expression or element iterator.
    typedef typename detail::ExpressionTraits<Derived>::Iterator Iterator;
    /// \brief Nested expression or element reference.
    typedef typename detail::ExpressionTraits<Derived>::Reference Reference;
    /// \brief Nested expression or element value type.
    typedef typename detail::ExpressionTraits<Derived>::Value Value;
...
#endif // !NDARRAY_Expression_hpp_INCLUDED
