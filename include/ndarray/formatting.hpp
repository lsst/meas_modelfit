 ...
 *  \brief Options for controlling stream output of Expression.
 */
class FormatOptions {
    int _width;
    int _precision;
...
public:
    ...
    void apply(std::ostream & os, Expression<Derived> const & expr) {
...
/// \brief Stream output for Expression using default-constructed FormatOptions.
template <typename Derived>
std::ostream & operator<<(std::ostream & os, Expression<Derived> const & expr) {
...
struct Formatter {
    static void apply(
        ...
        std::ostream & os, 
        Expression<Derived> const & expr,
        int level
    ) {
        os << options._open;
        if (!expr.empty()) {
            typename Expression<Derived>::Iterator const end = expr.end();
            typename Expression<Derived>::Iterator iter = expr.begin();
            Formatter<typename Expression<Derived>::Reference>::apply(options,os,*iter,level+1);
