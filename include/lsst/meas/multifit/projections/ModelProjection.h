...
public:
    ...
    /**
     * \brief Return the number of parameters that specify the coordinate 
     * transformation.
     */
     ...
     *
     *  Each of these computes the Footprint-compressed product as a row-major 
     *  array with the last dimension corresponding to the Footprint pixel 
     *  index and the first dimension (if any) corresponding to the parameter
     *  index.
     */
    //@{
     ...
     *
     *  The compute[Product]() member functions can be set to fill and return
     *  externally allocated (and possibly shared) data.  After setting a 
     *  buffer, only the ModelProjection should be allowed to modify it.
     */
    //@{
    ...
    /**
     * \brief Notify the ModelProjection of additional products it will be 
     * expected to compute.
     */
    ...
    /**
     * \brief Notify the ModelProjection that it will not be expected to 
     * compute certain products.
     */
    ...
    /**
     * \brief Return a bitwise OR of the products the ModelProjection is able 
     * to compute.
     */
    ...
    /**
     * \brief Return the Footprint the image representation will be computed 
     * on.
     */
...
protected:
    ...
    /**
     * \brief Construct blank projection, to be initialized later 
     * see Model::makeProjection().
     */
    ModelProjection() : _activeProducts(0), _validProducts(0), _photFactor(0) {}
    ...
    /**
     * \brief Set the PSF the image representation the model will be 
     * convolved with.
     */
    virtual void _setKernel(Kernel::ConstPtr const & kernel) = 0;

    /**
     * \brief Enable additional products, and return a bitflag of those 
     * successfully enabled.
     */
    virtual int _enableProducts(int toAdd) = 0;

    /**
     * \brief Disable products, and return a bitflag of those successfully 
     * disabled.
     */
    virtual int _disableProducts(int toRemove) = 0;

    /**
     * \brief Handle a linear parameter change broadcast from the associated 
     * Model.
     *
    ...
    /**
     * \brief Handle a nonlinear parameter change broadcast from the associated 
     * Model.
     *
     * Subclasses which override must call the base class implementation.
     */
...
}}}} // namespace lsst::meas::multifit::projections
