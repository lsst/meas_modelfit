 ...
 * memory belonging to Python objects in C++ Python extensions).  Reference counting
 ...
 * pointers; a const reference to a non-const array (Array<double,2> const &)
 * allows the elements of the array to be modified, while a non-const reference to
 * a const array (Array<const double,2> &) does not.  However, a
 * non-const reference to an array does allow the referenced array to be "reset"
 * to point to a new block of memory with a different shape and strides.  As
 * a result, functions should generally use const references for both input and
 * output array arguments (with const and non-const arrays, respectively).
 * Non-const arrays are implicitly (and efficiently) convertible to const arrays.
 * A const array can be cast to a non-const array using the const_array_cast function.
 *
 ...
 * dimensions.  The static_array_cast and dynamic_array_cast functions can be used to create
 * arrays with N>M RMC dimensions (the dynamic_array_cast returns an empty Array if the
 * strides are not appropriately RMC).
 *
 * \section views Indexing and Views
 * An Array (and by extension, ArrayView) with dimension N>1 behaves like largely like 
 ...
 *
 * For conciseness, I assume "using namespace ndarray" below, even though that
 * isn't actually the recommended use pattern.  Unless otherwise noted, all names
 * below (including nested namespaces) are part of namespace ndarray.
 *
 ...
 * \subsection deep_assignment Deep Assignment
 * Unlike the shallow, reference-counted copy constructor, regular array
 * assignment is deep, and requires that the input and output arrays have
 * identical dimensions:
 * \code
 * Array<double,2,2> a = allocate(makeVector(4,5));
 * // initialize 'a' to something meaningful here
 * Array<double,2,2> b = allocate(a.getShape());
 * b = a; // 'b' is now a deep copy of 'a'
 * \endcode
 * One cannot simply assign to an empty, default-constructed array:
 * \code
 * Array<double,2,2> a = allocate(makeVector(4,5));
 * // initialize 'a' to something meaningful here
 * Array<double,2,2> b;
 * b = a; // this doesn't work, because 'b' has shape (0,0) and null memory.
 * All the standard augmented assignment operators are defined
 * for Array objects, and work the way you'd expect:
 * \code
 * Array<double,2,2> a = allocate(makeVector(4,5));
 * // initialize 'a' to something meaningful here
 * a += a; // 'a' is now twice its previous value
 * \endcode
 * Assignment and augmented assignment between arrays and scalars is also
 * defined.  We could have written the previous example as:
 * Array<double,2,2> a = allocate(makeVector(4,5));
 * // initialize 'a' to something meaningful here
 * a *= 2.0; // 'a' is now twice its previous value
 * \subsection shallow_assignment Shallow Assignment
 * To reset an Array with new shape, strides, and memory, we use the
 * shallow "operator":
 * \code
 * Array<double,2,2> a = allocate(makeVector(4,5));
 * Array<double,2,2> b;
 * shallow(b) = a; // 'b' is now a view into 'a'
 * \endcode
 * 
 ...
 * Array<int,2,2>::Reference::Reference b2 = b[2]; // b1 == 8; Reference::Reference is int&.
 * // make 'c' into a copy of the third row of 'a' (Value is also a typedef to Array<int,1,1>):
 * Array<int,2,2>::Value c = allocate(makeVector(a.getSize<1>())); // a.getSize<1>() == 3
 * \endcode
 * Note that the "Reference" and "Value" typedefs are the same for multidimensional arrays, but refer
 * to element references and values for one-dimensional arrays.  The Element typedef is the same
 * for all dimensions.
 *
 * Array is designed to have extremely lightweight, fast nested iterators, with types provided
 ...
 * are also provided, though the const variants are not actually const (because of the way Array
 * handles constness).
 *
 * \section views Views
