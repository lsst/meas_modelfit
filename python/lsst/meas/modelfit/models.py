...
modelRegistry = lsst.pex.config.makeRegistry(
    ...
    as its first argument) that returns a lsst.meas.modelfit.Model.  It also takes
...
class BulgeDiskModelConfig(lsst.pex.config.Config):
    ...
    def makeModel(config):
        ...
        if config.bulgeRadius is None:
            basisVector = modelfitLib.Model.BasisVector()
            basisVector.append(disk)
            basisVector.append(bulge)
            prefixes = modelfitLib.Model.NameVector()
            prefixes.append("exp.")
            prefixes.append("dev.")
            return modelfitLib.Model.make(basisVector, prefixes, getCenterEnum(config))
        else:
            basis = disk
            bulge.scale(config.bulgeRadius)
            basis.merge(bulge)
            return modelfitLib.Model.make(basis, getCenterEnum(config))
