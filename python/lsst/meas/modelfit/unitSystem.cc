...
PYBIND11_PLUGIN(unitSystem) {
    ...
    clsUnitSystem.def(
        py::init<afw::coord::Coord const &, std::shared_ptr<afw::image::Calib const>, double>(),
        "position"_a, "calibIn"_a, "flux"_a
    );
    clsUnitSystem.def(
        py::init<afw::coord::Coord const &, Scalar>(),
        "position"_a, "mag"_a
    );
    clsUnitSystem.def(
    ...
    clsLocalUnitTransform.def(
        py::init<afw::coord::Coord const &, UnitSystem const &, UnitSystem const &>(),
