...
BOOST_AUTO_TEST_CASE(ExponentialModelProjection) {
    ...
    multifit::Model::Ptr sgModel = multifit::ModelFactory::createExponentialModel(
        flux, centroid, axes
    );

    BOOST_CHECK_EQUAL(sgModel->getLinearParameterSize(), 1);
    BOOST_CHECK_EQUAL(sgModel->getNonlinearParameterSize(), 5);
