...
compareRelative(
    ndarray::Expression<Derived1> const & a, 
    ndarray::Expression<Derived2> const & b,
...
compareAbsolute(
    ...
    if (ndarray::all((a-b) < tolerance && (b-a) < tolerance)) return true;
    boost::test_tools::predicate_result r(false);
    std::ostringstream oss;
    oss << "\n" << a << "\n != \n" << b << "\n";
    r.message() << oss.str();
